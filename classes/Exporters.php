...
class Exporters {
    ...
    public function searchPlayer($search){
        ...
        if ($search->has('playerId')) {
            $result[] = "roster.id = '" . $search['playerId'] . "'";
        }
        if ($search->has('player')) {
            $result[] = "roster.name = '" . $search['player'] . "'";
        }
        if ($search->has('team')) {
            $result[] = "roster.team_code = '" . $search['team']. "'";
        }
        if ($search->has('position')) {
            $result[] = "roster.pos = '" . $search['position'] . "'";
        }
        if ($search->has('country')) {
            $result[] = "roster.nationality = '" . $search['country'] . "'";
        }
    ...
    function getPlayerStats($search) {
        ...
        $player = $this->searchPlayer($search);

        $player = implode(' AND ', $player['result']);
        $sql = "
